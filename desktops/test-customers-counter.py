...
# Load counter if it exists
counter = []
try:
    if os.path.exists("./desktops/json/counter.json") and os.path.getsize("./desktops/json/counter.json") > 0:
        with open("./desktops/json/counter.json", "r") as file:
            counter = json.load(file)
except json.JSONDecodeError:
    print("Error loading JSON file. Starting with an empty counter.")
    counter = []
                ...
                if face_encoding:
                    ...
                    if (x > self.detection_box[0] and y > self.detection_box[1] and (x + w) < self.detection_box[2] and (y + h) < self.detection_box[3]):
                        if name not in current_faces_in_box:
                            ...
                            with open("./desktops/json/counter.json", "w") as file:
                                json.dump(counter, file)
                            current_faces_in_box.add(name)

                    cv2.putText(frame, f'{customers[name]["age"]}, {customers[name]["gender"]}, {expression}', (x, y-10), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 255), 2)
