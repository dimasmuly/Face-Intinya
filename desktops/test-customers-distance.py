...
class FaceRecognitionApp:
    ...
    def face_data(self, image, distance=76.2): 
    ...
    def ref_image_data(self):
        ...
        focal_length_found = self.focal_length_finder(
            76.2, 14.3, ref_image_face_width
        ) 
    ...
    def update(self):
        ...
        if ret:
            # Calling face_data function to find the width of face (pixels) in the frame for distance calculation 
            ...
            if face_width_in_frame != 0:
                distance = self.distance_finder(
                    self.ref_image_data(), 14.3, face_width_in_frame
                )
            ...
            for (x, y, w, h) in faces:
                ...
                if face_encoding:
                    ...
                    if name != "Unknown" and (distance / 3.4) <= 100.0:
                        cv2.putText(frame, f'{customers[name]["age"]}, {customers[name]["gender"]}, {expression}, {round(distance / 3.4, 2)}', (x, y-10), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 255), 2)
                        # Puttext di bawah kotak wajah
                        cv2.putText(frame, name, (x, y+h+20), cv2.FONT_HERSHEY_SIMPLEX, 0.2, (255, 255, 255), 1)
                    else:
                        cv2.putText(frame, f'Unknown, {round(distance / 3.4, 2)} To long distance!', (x, y-10), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 255), 2)
